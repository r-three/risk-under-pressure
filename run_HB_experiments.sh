...
#   ATTACKS="gcg"            bash run_HB_experiments.sh   # just the slow one
#   ATTACKS="pair jailbroken" bash run_HB_experiments.sh   # just the fast ones
#
# NOTE ON JOB NAMES: these now carry the attack (rup_HB_<model>_<attack>_s<seed>). Runs
...
BASE="python scripts/run_inference.py --experiment configs/experiments/base.yaml --benchmark harmbench --output-dir $RUN_ROOT --judge-model $JUDGE --resume"

# Static attacks only. The RL/GRPO adaptive attack lives in run_rl_HB_experiments.sh
...
submit_model() {
    ...
    for atk in $ATTACKS; do
        submit "rup_HB_${short}_${atk}_s${seed}$JUDGE_TAG" \
...
echo "Watch:  squeue -u $USER --noheader -o '%j %t' | grep rup_HB"
