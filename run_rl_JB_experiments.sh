...
# 1.4M target generations, 4.1M judge passes and 680k 7B backward passes — before the
# group-boundary early stop trims it, which it will for every behavior that jailbreaks.
#
# n-prompts=100 is the full JailbreakBench set; 17 targets x 10 seeds is the full grid. Trim with
...
RL_JB="python scripts/run_inference.py --experiment configs/experiments/base.yaml --benchmark jailbreakbench --attack rl --n-prompts 100 --rl-groups 2 --output-dir $RUN_ROOT --judge-model $JUDGE --resume"
...
# Override to run a subset:  SEEDS="1394 42" bash run_rl_JB_experiments.sh
...
submit_rl() {
    ...
    for seed in $SEEDS; do
        submit "rup_JB_rl_${short}_s${seed}$JUDGE_TAG" \
            "$RL_JB --model $cfg --seeds $seed"
    done
}
